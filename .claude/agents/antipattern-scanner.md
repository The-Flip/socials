...
- **Mixed Concerns**: Core logic mixed with logging, I/O, validation, etc.
...
self.client = SpecificApiClient("hardcoded-params")
...
```python
# DETECT: Repeated processing logic
totals = sum(x.count for x in items)
average = totals / len(items)
...
def run_report(self):
    # report code...
    self.hardcoded_step_1()  # No way to customize
    self.hardcoded_step_2()  # Must modify for new behavior
```
...
token = config.get("access_token", "")  # Should be explicit / fail loud
channel = config.get("channel_id", None)  # Hides missing config
...
class ReportHandler:  # What does it handle?
...
mock_client = Mock()
mock_client.fetch.return_value = [1, 2, 3]  # Fake data, not real testing
...
- Look for existing test patterns to follow
- Suggest exercising actual code instead of mocking it
